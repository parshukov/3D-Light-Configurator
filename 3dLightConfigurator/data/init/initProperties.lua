...
function globalProperty(name)
    local ref = sasl.findDataRef(name)
    if not ref then
        return nil
    end

    local get = function(self) return sasl.getDataRef(ref); end
...
function globalPropertys(name)
    ...
    elseif t == TYPE_FLOAT or t == TYPE_INT or t == TYPE_DOUBLE then
        get = function(self) return tostring(sasl.getDataRef(ref)); end
        set = function(self, value) end
        logDebug('"'..name..'": '.."Casting "..propTypeToString(t).." to string")
    else
...
function createGlobalPropertys(name, default, isNotPublished, isShared)
    ...
    if default ~= nil then sasl.setDataRef(ref, default) elseif isShared then sasl.setDataRef(ref, '') end
    return {
        __p = 1;
        name = name;
        get = function(self) return sasl.getDataRef(ref); end;
