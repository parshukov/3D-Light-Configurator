...
function setCursor(x, y, shape, layer)
    if layer == 1 then
        ...
        cursor.x = sasl.getCSMouseXPos()
        cursor.y = sasl.getCSMouseYPos()
    end
...
-- Draw cursor shape
function drawCursor(inCursor)
    if inCursor.shape and inCursor.shape.shape then
        sasl.gl.drawTexture(inCursor.shape.shape,
                inCursor.x + inCursor.shape.x, inCursor.y + inCursor.shape.y,
                inCursor.shape.width, inCursor.shape.height,
                { 1.0, 1.0, 1.0, 1.0 })
    end
end
...
function setFocusedPath(path)
    ...
    if currentFocusedComponentPath then
        for _, c in pairs(currentFocusedComponentPath) do
            set(c.focused, false)
        end
    end
